...
export default function People() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await fetch("/api/db/users", { cache: "no-store" });
  ...
  return (
    ...
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            <CardTitle>수강생</CardTitle>
            <CardDescription>수강생 목록입니다</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {data.map((assignment) => (
              <div key={assignment._id} className="flex items-center gap-4 p-3 rounded-lg border border-border">
                <div className="p-2 bg-muted rounded-lg">
                  <FileText className="h-4 w-4 text-muted-foreground" />
                </div>
                <div className="flex-1">
                  <h3 className="font-medium text-foreground">{assignment.이름}</h3>
                  <p className="text-sm text-muted-foreground">{assignment.학번}</p>
                </div>
                <div className="text-right">
                  <Badge variant={assignment.구분 === "student" ? "teacher" : "secondary"}>
                    {assignment.구분 === "student" ? "학생" : "교사"}
                  </Badge>
                  <p className="text-xs text-muted-foreground mt-1">{assignment.입학년도}</p>
                </div>
              </div>
            ))}
          </CardContent>
      ...
      <ul className="list-disc pl-5">
        {data.map((p, idx) => (
          <li key={p._id ?? idx}>{p.이름 ?? "(no name)"}</li>
        ))}
      </ul>
    </div>
  );
